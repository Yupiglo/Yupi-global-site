...
export default function EditPortfolioPage() {
    ...
    useEffect(() => {
        ...
        api.portfolio.getAll()
            .then(items => {
                const item = items.find((p: any) => p.id === parseInt(id));
                if (item) {
                    setFormData({
                        title: item.title,
                        slug: item.slug,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSaving(true);
        try {
            await api.portfolio.update(parseInt(id), formData);
            setSuccess(true);
        ...
        <div className="max-w-4xl mx-auto space-y-6">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <Link href="/portfolio" className="p-2 hover:bg-card rounded-lg transition-colors border border-transparent hover:border-border group">
                        <ArrowLeft className="h-5 w-5 text-muted-foreground group-hover:text-foreground" />
                    </Link>
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight">Éditer le Projet</h1>
                        <p className="text-muted-foreground text-sm">Modifiez les détails de votre réalisation.</p>
                    </div>
                </div>
                <button
                    type="submit"
            ...
            <form id="portfolio-form" onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-6">
                    <div className="rounded-xl bg-card p-6 shadow-sm border border-border space-y-4 transition-colors duration-300">
                        <div>
                            <label htmlFor="title" className="block text-sm font-bold text-muted-foreground mb-1">Nom du projet</label>
                            <input
                                required
                                type="text"
                                id="title"
                ...
                <div className="space-y-6">
                    <div className="rounded-xl bg-card p-6 shadow-sm border border-border space-y-4 transition-colors duration-300">
                        <h3 className="font-bold border-b border-border pb-2">Détails</h3>
                        <div>
                            <label htmlFor="category" className="block text-sm font-medium text-muted-foreground mb-1">Catégorie</label>
                            <input
                                type="text"
                                id="category"
                                name="category"
