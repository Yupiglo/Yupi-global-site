...
// Shared file path - using absolute path to ensure both frontend and admin access the same file
...
export async function PUT(request: Request) {
    try {
        ...
        fs.writeFileSync(CONFIG_FILE_PATH, JSON.stringify(body, null, 2));
        return NextResponse.json({ success: true, data: body });
    } catch (error) {
        console.error("Error writing banner config:", error);
